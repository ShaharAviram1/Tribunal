...
| 3 | The case page | No new model behaviour; no storage change. | A cold reader, from the case page alone, states each verdict, one reason each, and whether all three agreed; the result recorded as a dated note. Failures render as failures. A fresh clone renders T-001 from the committed run with no key. |
