...
| Gavel | Fires once, on transition to a terminal job with at least one returned opinion. Amended 2026-09-05: the transition is acted on only while the page is attended, visible and in a focused window; seen otherwise, the reveal and the gavel wait for the viewer to return, with one line under the header, "The bench is ready. Return to this page to hear it.", because a hidden tab defers media loading and an unfocused one plays the gavel to an empty room |
